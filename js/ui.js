...
const getCategoriaOptionsHTML = (selecionada = null) => {
    ...
    if (!categorias.length) return '<option disabled>Nenhuma categoria cadastrada</option>';
    ...
    container.innerHTML = `
        <div class="p-3">
            ...
            <div id="panel-chart-view">
                <div style="height: 400px; position: relative;"><canvas id="annual-mixed-chart"></canvas></div>
...
const renderDetailedTable = () => {
    ...
    container.innerHTML = `
        <table class="table table-bordered table-sm table-hover" style="font-size: 0.85rem;">
            <thead style="position: sticky; top: 0; z-index: 2;"><tr><th class="bg-light" style="min-width: 120px;">Categoria</th>${headerCols}</tr></thead>
            <tbody>
                <tr class="table-success"><td colspan="13"><strong>RECEITAS</strong></td></tr>${rowsReceitas || '<tr><td colspan="13" class="text-center text-muted">Sem dados</td></tr>'}
                <tr class="table-danger border-top"><td colspan="13"><strong>DESPESAS</strong></td></tr>${rowsDespesas || '<tr><td colspan="13" class="text-center text-muted">Sem dados</td></tr>'}
...
export const renderContas = () => {
    ...
    const headerHtml = `
        <div class="d-flex justify-content-end mb-3">
            <button id="btn-manage-categories" class="btn btn-outline-primary btn-sm"><i class="fas fa-tags me-2"></i>Gerenciar Categorias</button>
        </div>`;
    ...
    const cardsHtml = contas.map(conta => {
        ...
        if (isTipoCartao(conta.tipo)) acoesEspecificas = `<button class="btn btn-outline-secondary btn-sm" data-action="ver-fatura" data-id="${conta.id}" title="Ver Fatura"><i class="fas fa-receipt fa-fw"></i></button>`;
        else acoesEspecificas = `<button class="btn btn-outline-secondary btn-sm" data-action="ver-extrato" data-id="${conta.id}" title="Ver Extrato"><i class="fas fa-chart-bar fa-fw"></i></button>`;
            ...
            <div class="card shadow-sm mb-2">
                <div class="card-body p-3">
                    <div class="d-flex align-items-center">
                        <div class="me-3 fs-2 text-primary opacity-75"><i class="${iconClass}"></i></div>
                        <div class="flex-grow-1"><div class="fw-bold">${conta.nome}</div><small class="text-body-secondary">${conta.tipo}</small></div>
                        <div class="text-end"><span class="fw-bold fs-5 ${saldo >= 0 ? 'income-text' : 'expense-text'}">${formatarMoeda(saldo)}</span></div>
                    </div>
                    <div class="d-flex justify-content-end gap-1 border-top pt-2 mt-2">${acoesEspecificas}${botoesGerais}</div>
                </div>
            </div>`;
    }).join('');
    ...
    container.innerHTML = `
        <div class="mb-3">
            ...
            <select id="tipo-compra" name="tipo_compra" class="form-select form-select-sm">
                <option value="vista">À Vista</option><option value="parcelada">Parcelada</option><option value="recorrente">Recorrente</option>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <select id="tipo-transacao" name="tipo" class="form-select form-select-sm">
                <option value="despesa" selected>Despesa (Débito)</option><option value="receita">Receita (Crédito)</option>
            </select>
        </div>
        ...
        <div id="parcelada-fields" class="extra-fields"><div class="mb-3"><label class="form-label">Nº de Parcelas</label><input name="numero_parcelas" type="number" min="2" class="form-control form-control-sm"></div></div>
        <div id="recorrente-fields" class="extra-fields">
            <div class="mb-3"><label class="form-label">Frequência</label>
                <select name="frequencia" class="form-select form-select-sm">
                    <option value="diaria">Diária</option><option value="quinzenal">Quinzenal</option><option value="mensal" selected>Mensal</option><option value="anual">Anual</option>
                </select>
            </div>
...
export const getBillModalContent = (id = null) => {
    ...
    const warning = isParcela ? `<div class="alert alert-info small"><i class="fas fa-info-circle"></i> Editando apenas esta parcela. Use a engrenagem na lista para editar a série.</div>` : '';
    const body = `
...
export const getInstallmentPurchaseModalContent = (compra) => {
    ...
    if (isCartao) {
        ...
        camposEspecificos = `
            <div class="row">
                <div class="col-6 mb-3"><label class="form-label">Frequência</label><select name="frequencia" class="form-select"><option value="mensal">Mensal</option><option value="quinzenal">Quinzenal</option><option value="semestral">Semestral</option><option value="anual">Anual</option></select></div>
                <div class="col-6 mb-3"><label class="form-label">Qtde Restante</label><input name="quantidade" type="number" value="12" class="form-control"></div>
            </div>
            <div class="mb-3"><label class="form-label">Data do Próximo</label><input name="data_inicio" type="date" value="${toISODateString(new Date())}" class="form-control" required></div>
...
export const getPayBillModalContent = (billId) => {
    ...
    const body = `
        <form id="form-pagamento" data-bill-id="${bill.id}" data-valor="${bill.valor}" data-desc="${bill.descricao}" data-cat="${bill.categoria || 'Contas'}">
            ...
            <div class="mb-3"><label class="form-label">Data</label><input type="date" name="data" value="${toISODateString(new Date())}" class="form-control"></div>
            <div class="mb-3"><label class="form-label">Conta</label><select name="conta_id" class="form-select">${getContas().map(c=>`<option value="${c.id}">${c.nome}</option>`).join('')}</select></div>
...
export const getTransactionModalContent = (id) => {
    ...
    return { title: 'Editar Transação', body };
};
...
export const renderFilters = (type, currentFilters = {}) => {
    ...
    container.innerHTML = `
        <div class="row g-2 mb-3">
            <div class="col-md-4"><select class="form-select form-select-sm" id="${type}-month-filter"><option value="todos" ${!currentFilters.mes || currentFilters.mes === 'todos' ? 'selected' : ''}>Todos os Meses</option>${monthOptions}</select></div>
            <div class="col-md-4"><select class="form-select form-select-sm" id="${type}-account-filter"><option value="todas" ${!currentFilters.contaId || currentFilters.contaId === 'todas' ? 'selected' : ''}>Todas as Contas</option>${accountOptions}</select></div>
            <div class="col-md-4"><input type="search" class="form-control form-control-sm" id="${type}-search-input" placeholder="Pesquisar..." value="${currentFilters.pesquisa || ''}"></div>
        </div>`;
};
...
const renderSummaryPanel = (containerId, items, type) => {
    ...
    container.innerHTML = `
        <div class="alert alert-light py-2"><div class="d-flex justify-content-around flex-wrap small text-center">
...
const renderTransactionCard = (t) => {
    ...
    const temSerie = t.compra_parcelada_id; 
    const extraBtn = temSerie ? `<button class="btn btn-outline-secondary btn-sm" data-action="recriar-compra-parcelada" data-id="${t.compra_parcelada_id}" title="Configurar Série"><i class="fas fa-cog"></i></button>` : '';
    ...
    if (isPendente) {
        ...
        actions = `<div class="btn-group"><button class="btn ${btnClass} btn-sm" data-action="pagar-conta" data-id="${t.id}" title="${btnTitle}"><i class="${btnIcon}"></i></button><button class="btn btn-outline-secondary btn-sm" data-action="editar-lancamento" data-id="${t.id}"><i class="fas fa-edit"></i></button>${extraBtn}<button class="btn btn-outline-danger btn-sm" data-action="deletar-lancamento" data-id="${t.id}" title="Apagar"><i class="fas fa-trash"></i></button></div>`;
    } else {
    ...
    return `
        <div class="accordion-item ${opacityClass}">
            <h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#${collapseId}">
                <div class="d-flex w-100 align-items-center">
                    <span class="transaction-icon-wrapper me-3" style="background-color:${icon.color};"><i class="${icon.icon}"></i></span>
                    <div class="flex-grow-1"><span>${t.descricao}</span><div class="small">${statusBadge}</div></div>
                    <span class="ms-auto fw-bold ${corValor}">${sinal} ${formatarMoeda(t.valor)}</span>
                </div>
            </button></h2>
            <div id="${collapseId}" class="accordion-collapse collapse"><div class="accordion-body d-flex justify-content-between align-items-center">
                <div><small class="text-body-secondary"><i class="fas fa-calendar-alt"></i> ${new Date(dataExibicao + 'T12:00:00').toLocaleDateString('pt-BR')} | <i class="fas fa-tag"></i> ${t.categoria} | <i class="fas fa-wallet"></i> ${conta ? conta.nome : 'N/A'}</small></div>
                ${actions}
            </div></div>
        </div>`;
};
...
const renderBillItem = (bill, compras) => {
    ...
    if (isParcela) {
        ...
        if(c) { cat = c.categoria; if (c.descricao && c.descricao.includes('(Série)')) isSerie = true; }
    }
    ...
    return `
        <div class="accordion-item">
            ...
            <div id="${collapseId}" class="accordion-collapse collapse"><div class="accordion-body d-flex justify-content-between align-items-center">
                <div><small class="text-body-secondary">Vencimento: ${new Date(bill.data_vencimento + 'T12:00:00').toLocaleDateString('pt-BR')}</small></div>
                <div class="btn-group">
...
export const renderVisaoMensal = () => {
    ...
    container.innerHTML = `
        ...
        <div class="row text-center mb-3">
            <div class="col-4"><h6>Receitas</h6><p class="h4 income-text mb-0">${formatarMoeda(metrics.rendaPrevistaTotal)}</p><small class="text-body-secondary" style="font-size:0.7rem">(Real: ${formatarMoeda(metrics.rendaRealizada)})</small></div>
            <div class="col-4"><h6>Despesas</h6><p class="h4 expense-text mb-0">${formatarMoeda(metrics.despesaPrevistaTotal)}</p><small class="text-body-secondary" style="font-size:0.7rem">(Real: ${formatarMoeda(metrics.despesaRealizada)})</small></div>
            <div class="col-4"><h6>Saldo</h6><p class="h4 ${metrics.saldoPrevisto >= 0 ? 'income-text':'expense-text'} mb-0">${formatarMoeda(metrics.saldoPrevisto)}</p></div>
        </div>
    ...
    const despesasMap = {};
    transacoes.filter(t => t.data.startsWith(mesAtual) && t.tipo === 'despesa').forEach(t => { despesasMap[t.categoria] = (despesasMap[t.categoria] || 0) + t.valor; });
    lancamentosFuturos.filter(l => l.data_vencimento.startsWith(mesAtual) && l.tipo === 'a_pagar' && l.status === 'pendente').forEach(l => { despesasMap[l.categoria] = (despesasMap[l.categoria] || 0) + l.valor; });
    
    if(ctx && Object.keys(despesasMap).length > 0) {
        summaryChart = new Chart(ctx, {type:'doughnut',data:{labels:Object.keys(despesasMap),datasets:[{data:Object.values(despesasMap),backgroundColor:CHART_COLORS}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}}}});
    }
};
...
export const renderFinancialHealth = () => {
    ...
    container.innerHTML = `
        <div class="row"><div class="col-12"><div class="card mb-3"><div class="card-body text-center"><h5 class="card-title">Score de Saúde Financeira</h5><div class="progress mx-auto my-3" style="height: 25px; max-width: 400px;"><div class="progress-bar bg-${scoreColor}" role="progressbar" style="width: ${metrics.financialScore.toFixed(0)}%;">${metrics.financialScore.toFixed(0)} / 100</div></div></div></div></div></div>
        <div class="row">
            <div class="col-lg-6 mb-3"><div class="card h-100"><div class="card-header"><h6 class="mb-0">Patrimônio Líquido</h6></div><ul class="list-group list-group-flush"><li class="list-group-item d-flex justify-content-between"><span>Ativos</span> <span class="text-success">${formatarMoeda(metrics.totalAtivos)}</span></li><li class="list-group-item d-flex justify-content-between"><span>Passivos</span> <span class="text-danger">${formatarMoeda(metrics.totalPassivos)}</span></li><li class="list-group-item d-flex justify-content-between"><strong>Total</strong> <strong>${formatarMoeda(metrics.patrimonioLiquido)}</strong></li></ul></div></div>
            <div class="col-lg-6 mb-3"><div class="card h-100"><div class="card-header"><h6 class="mb-0">Evolução do Patrimônio</h6></div><div class="card-body"><canvas id="net-worth-chart"></canvas></div></div></div>
        </div>`;
    if (netWorthChart) netWorthChart.destroy();
    const nwCtx = document.getElementById('net-worth-chart')?.getContext('2d');
    if (nwCtx && metrics.historicoPatrimonio.length) { netWorthChart = new Chart(nwCtx, {type: 'line', data: {labels: metrics.historicoPatrimonio.map(h => h.mes), datasets: [{label: 'Patrimônio', data: metrics.historicoPatrimonio.map(h => h.valor), borderColor: '#4A5568', tension: 0.1, fill: false}]}, options: {responsive: true, maintainAspectRatio: false}}); }
};
...
export const renderMonthlyStatementDetails = (mes) => {
    ...
    container.innerHTML = `<div class="row mb-4 g-3 text-center"><div class="col-md-4"><div class="card h-100 border-success shadow-sm"><div class="card-header bg-success text-white py-2 fw-bold">Entradas</div><div class="card-body"><h4 class="card-title text-success mb-0">${formatarMoeda(entradas)}</h4></div></div></div><div class="col-md-4"><div class="card h-100 border-danger shadow-sm"><div class="card-header bg-danger text-white py-2 fw-bold">Saídas</div><div class="card-body"><h4 class="card-title text-danger mb-0">${formatarMoeda(saidas)}</h4></div></div></div><div class="col-md-4"><div class="card h-100 border-${resultado >= 0 ? 'success' : 'danger'} shadow-sm"><div class="card-header bg-${resultado >= 0 ? 'success' : 'danger'} text-white py-2 fw-bold">Balanço</div><div class="card-body"><h4 class="card-title text-${resultado >= 0 ? 'success' : 'danger'} mb-0">${formatarMoeda(resultado)}</h4></div></div></div></div><h6 class="border-bottom pb-2 mb-3 text-body-secondary d-flex justify-content-between align-items-center"><span>Movimentações</span><small class="text-muted fw-normal"><span class="badge bg-warning text-dark">Pendente</span> = Agendado</small></h6><div class="accordion" id="statement-accordion">${listHtml}</div>`;
};
